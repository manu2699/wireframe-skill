...
export function Nav(props: { node: WFNode }) {
  ...
  const navCls = isTop
    ? "wf-ia-nav flex flex-row gap-2 items-center flex-wrap"
    : "wf-ia-nav flex flex-col gap-3.5 w-[240px] shrink-0 self-start overflow-y-auto max-h-[100cqh]";
  ...
  return (
    <nav className={navCls}>
      {props.node.groups?.map((g, gi) => (
        <div
          key={gi}
          className={isTop ? "flex flex-row items-center gap-1.5" : "flex flex-col gap-1"}
        >
          {g.label && <div className="wf-nav-group-label pt-0.5 px-0.5 pb-0">{g.label}</div>}
          {g.items.map((it: NavItem & { _id?: string }, ii) => {
            const item = (
              ...
              >
                {sketch && <SketchBorder strokeWidth={it.active ? 2.2 : 1.4} roughness={1.4} />}
                <Pin id={it._id} />
